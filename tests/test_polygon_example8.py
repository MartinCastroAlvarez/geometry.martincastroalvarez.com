...
def test_example8_full_pipeline_requires_nine_guards():
    ...
    job_stitch = Job(id=Identifier("ex8-s"), step_name=StepName.STITCHING, stdin=dict(EXAMPLE8_STDIN), stdout=dict(stdout))
    stdout.update(StitchingStep(job=job_stitch, user=_user()).run())
    job_ear = Job(id=Identifier("ex8-e"), step_name=StepName.EAR_CLIPPING, stdin=dict(EXAMPLE8_STDIN), stdout=dict(stdout))
    stdout.update(EarClippingStep(job=job_ear, user=_user()).run())
    job_convex = Job(id=Identifier("ex8-c"), step_name=StepName.CONVEX_COMPONENT_OPTIMIZATION, stdin=dict(EXAMPLE8_STDIN), stdout=dict(stdout))
