...
class ArtGalleryPublishMutation(PrivateMutation[ArtGalleryPublishMutationRequest, dict[str, Any]]):
    """Publish gallery from job stdout; user must own the job."""

    def _validate_body(self, body: dict[str, Any]) -> ArtGalleryPublishMutationRequest:
        return ArtGalleryPublishMutationRequest(job_id=Identifier(body.get("job_id")))

    def mutate(self, validated_input: ArtGalleryPublishMutationRequest) -> dict[str, Any]:
        ...
        index.save(Indexed(index_id=Identifier(Countdown.from_timestamp(gallery.created_at)), real_id=gallery.id))
