...
class Interval(Element1D, Serializable):
    ...
    def unserialize(cls, data: dict[str, Any]) -> Interval:
        if not isinstance(data, dict):
            raise SerializedInvalidDictError(f"Interval.unserialize expects a dict, got {type(data).__name__}")
        ...
        try:
            start = data["start"] if isinstance(data["start"], Decimal) else Decimal(str(float(data["start"])))
            end = data["end"] if isinstance(data["end"], Decimal) else Decimal(str(float(data["end"])))
        except (TypeError, ValueError) as error:
            raise IntervalInvalidError(f"Interval.unserialize invalid start/end: {error}") from error
    ...
    def validate(self) -> None:
        if not isinstance(self.start, Decimal):
            raise IntervalInvalidError(f"start must be a Decimal, got {type(self.start).__name__}")
        if not isinstance(self.end, Decimal):
            raise IntervalInvalidError(f"end must be a Decimal, got {type(self.end).__name__}")
        if self.end < self.start:
            raise IntervalInvalidError(f"Interval end ({self.end}) must be >= start ({self.start})")
    ...
    def intersects(self, obj: Element, inclusive: bool = True) -> bool:
        if isinstance(obj, Interval):
            ...
            return self[0] < obj[1] and obj[0] < self[1]
        raise NotImplementedError(f"Interval.intersects not implemented for {type(obj)}")
