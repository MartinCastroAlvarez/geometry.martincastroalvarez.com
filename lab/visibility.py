...
class Visibility(Generic[T]):
    def __init__(self, items: dict[Hash, set[T]] | None = None) -> None:
        ...
        if items is not None:
            if not isinstance(items, dict):
                raise VisibilityInvalidItemsError(f"Visibility items must be a dictionary: {items!r}")
            if not all(isinstance(key, Hash) for key in items.keys()):
                raise VisibilityInvalidItemsError(f"Visibility items must have Hash keys: {items!r}")
            if not all(isinstance(value, set) for value in items.values()):
                raise VisibilityInvalidItemsError(f"Visibility items must have set values: {items!r}")
            self.items = items
    ...
    def best(self) -> Hash:
        ...
        if len(self.items[best]) == 0:
            raise GuardCoverageFailureError(f"Best guard cannot see any remaining component: {self.items}.")
        return best
